...
import { getChampionOdds, getScorecard, getTopScorers, MODEL_CALL } from '../../data/api.js'
    ...
    <div className="space-y-5">
      ...
      <Card accent="purple" title="Model Call" icon={Trophy}>
        <p className="text-lg leading-snug text-turf-900">
          <Flag team="France" w={26} />{' '}
          <b className="font-display text-wc-purple">{MODEL_CALL.champion}</b> are the predicted
          champions <span className="font-bold text-turf-600">({MODEL_CALL.championOdds}%)</span>.
          {' '}The host <Flag team="United States" w={22} />{' '}
          <b>United States</b> is <b className="text-wc-red">not winning this</b> — a{' '}
          <b>{MODEL_CALL.usaOdds}%</b> title shot with a ceiling at the{' '}
          <b>{MODEL_CALL.usaCeiling}</b>, eliminated before the final.
        </p>
      </Card>

      {/* KPI row */}
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <StatCard label="Predicted Champion" value="France" sub="11.8% to win" accent="purple" />
        <StatCard label="Host (USA)" value="8.4%" sub="ceiling: semifinals" accent="red" />
